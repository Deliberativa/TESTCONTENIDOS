(()=>{
  ...
  const INSTALLATION_PHOTOS={
    ...
    'P.E. OIZ':{src:'assets/photos/installations/oiz-ermita-san-cristobal.png',alt:'Ermita de San Cristóbal y aerogeneradores del parque eólico de Oiz',description:'Ermita de San Cristóbal y parque eólico de Oiz',sourceLabel:'Imagen aportada al proyecto',expandable:true,preserveOrthophoto:true},
    'URIARTE SAFYBOX, S.A.':{
      ...
      technical:{
        ...
        citation:'(Interempresas / Futurenergy, 2014).',
        facts:[
    ...
    'ANARBE I (PIE DE PRESA)':{src:'assets/photos/installations/anarbe-presa-central-hidroelectrica.jpeg',alt:'Presa de Añarbe y central hidroeléctrica a pie de presa',description:'Presa de Añarbe, en Errenteria, con una central hidroeléctrica a pie de presa.',sourceLabel:'Imagen aportada al proyecto',expandable:true,preserveOrthophoto:true},
    'SOBRON 1':{src:'assets/photos/installations/sobron-presa-central-hidroelectrica.jpeg',alt:'Presa de Sobrón y central hidroeléctrica en Álava',description:'Presa de Sobrón, en Álava, vinculada a la central hidroeléctrica de Sobrón.',sourceLabel:'Imagen aportada al proyecto',expandable:true,preserveOrthophoto:true},
    'BARAZAR 1':{
    ...
    'EKINDAR AZPEITIA':{
      src:'assets/photos/installations/ekindar-azpeitia-urrestilla.webp',
      alt:'Vista aérea de la planta solar fotovoltaica Ekindar en Urrestilla, Azpeitia',
      ...
      sourceLabel:'KREAN / NAIZ',
      pageUrl:'https://www.naiz.eus/es/info/noticia/20241011/las-ekiolas-de-mendialdea-azpeitia-y-leintz-bailara-ya-generan-luz',
      linkUrl:'https://www.naiz.eus/es/info/noticia/20241011/las-ekiolas-de-mendialdea-azpeitia-y-leintz-bailara-ya-generan-luz',
      expandable:true,
      preserveOrthophoto:true,
      technical:{
  ...
  window.renderEnergyInstallationsMap=(row,cfg)=>{
    ...
    const focusedTechnologyKey=TECHNOLOGIES[cfg.onlyTechnology]?cfg.onlyTechnology:null,onlyEveSolar=Boolean(cfg.onlyEveSolar),showBothSolar=Boolean(cfg.showBothSolar),substationFocus=Boolean(cfg.onlySubstations),storageFocus=Boolean(cfg.onlyStorage),offshoreWindFocus=Boolean(cfg.onlyOffshoreWind);
    const records=(substationFocus||storageFocus||offshoreWindFocus?[]:installations.features).map(feature=>{
      ...
      return{feature,properties,mw,units,key,layerKey,isEveSolar,isEsiosSolar,isDocumentedSelfConsumption:Boolean(properties.documentedSelfConsumption),cogenFuelClass:cogenEvidence?.fuelClass||null,cogenEvidence,coordinate:feature.geometry.coordinates};
    });
    ...
    const metadata=TECHNOLOGY_ORDER.map(key=>{
      const subset=mapRecords.filter(d=>d.layerKey===key&&!((cfg.scenario==='tendential'||showBothSolar)&&d.isEveSolar));
    ...
    ptsPotentialCapacityByTechnology.wind+=windRepoweringAdditionalMW;ptsPotentialGenerationByTechnology.wind+=windRepoweringAdditionalGWh;if(!cfg.targetGenerationByTechnology)ptsPotentialBarTechnologyEnergy.wind+=windRepoweringAdditionalGWh*deliveryFactor;
    ...
    const energyShareTitle=energyShare.querySelector('.map-energy-share-title');energyShareTitle.innerHTML='<strong>Total</strong>';
  ...
  detail.prepend(button);
    ...
    const markers=operatingMarkerLayer.selectAll('circle').data(ordered).join('circle')
      .attr('class',d=>`energy-marker${d.key==='cogen'?` cogen-fuel-${d.cogenFuelClass}`:''}`).attr('cx',d=>markerPosition(d)[0]).attr('cy',d=>markerPosition(d)[1])
      .attr('r',d=>d.isEveSolar?Math.max(5,d.mw<0.1?3.4:radius(d.mw)):d.key==='wind'?windCapacityRadius(d.mw):d.mw<0.1?2.3:radius(d.mw)).attr('fill',d=>d.isEveSolar?EVE_SOLAR_COLOR:d.layerKey==='solar'?ESIOS_SOLAR_COLOR:d.key==='cogen'?cogenMarkerFill(d):TECHNOLOGIES[d.key].color).attr('fill-opacity',.95).attr('stroke',d=>d.isEveSolar?EVE_SOLAR_STROKE:d.layerKey==='solar'?ESIOS_SOLAR_STROKE:'#fff').attr('stroke-width',d=>d.isEveSolar?1.6:.8)
      .attr('tabindex',0).attr('role','button').attr('aria-label',d=>`${d.properties.descripcion||'Instalación'}; ${d.isEveSolar?'Solar a red · declarado por EVE':d.isDocumentedSelfConsumption?(d.properties.selfConsumptionLabel||'Autoconsumo documentado'):d.key==='cogen'?COGEN_FUEL_CLASSES[d.cogenFuelClass].label:TECHNOLOGIES[d.key].label}; ${formatNumber(d.mw,Math.max(0,d.mw<1?3:1))} MW; ${cleanMunicipality(d.properties.municipio)}`);
    ...
    const ptsPotentialMarkers=ptsPotentialLayer.selectAll('circle').data([...ptsPotentialRecords].sort((a,b)=>b.mw-a.mw)).join('circle')
      ...
      .attr('r',d=>d.key==='wind'?windCapacityRadius(d.mw):ptsPotentialRadius(d.mw)).attr('fill',d=>patternUrl(`pts-potential-${d.key}-grid`))
    ...
    const offshoreWindPostPtsMarkers=offshoreWindPostPtsLayer.selectAll('g').data(OFFSHORE_WIND_POST_PTS).join('g')
      .attr('class','offshore-wind-post-pts-marker').attr('transform',d=>`translate(${projection(d.coordinate).join(',')})`)
      .attr('tabindex',0).attr('role','button')
      .attr('aria-label',d=>`${d.name}; eólica marina flotante en ensayo; ${d.statusLabel}; ${d.scope}`);
    offshoreWindPostPtsMarkers.append('circle').attr('class','offshore-wind-post-pts-disc').attr('r',13);
    offshoreWindPostPtsMarkers.append('path').attr('class','offshore-wind-post-pts-turbine').attr('d','M0 1 V10 M-4 10 H4 M0 1 L0 -8 M0 1 L-7 5 M0 1 L7 5');
    offshoreWindPostPtsMarkers.append('circle').attr('class','offshore-wind-post-pts-hub').attr('r',1.7);
    offshoreWindPostPtsMarkers.append('title').text(d=>`${d.name} · Eólica marina flotante en ensayo · ${d.statusLabel}. No es un parque comercial aprobado.`);
    ...
    const useEstimatedGatewayFlows=isTendentialScenario&&!timelineEnabled;
    const PENINSULAR_FLOW_MAX_GWH=Number(window.energyMapPeninsularFlowMaxGWh)||16026;
    const estimatedGatewayFlowByName={
    ...
    const importGatewayRecords=clusteredImportGatewayRecords.map(node=>{
      const estimatedFlow=useEstimatedGatewayFlows&&!node.isFuture?estimatedGatewayFlowByName[node.name]||null:null,
      ...
      return{...node,estimatedFlow,estimatedAnnualGWh:estimatedFlow?.gwh||null,flowRenewableShare:estimatedFlow?.renewableShare??null,displayCapacityMVA,position,angle,symbolScale,thicknessScale,labelDy:-10*symbolScale};
    });
    ...
    const gatewaySymbolTransform=(d,zoomScale=1,capacity=d.displayCapacityMVA)=>{
      const length=d.estimatedFlow ? .43 : importGatewayLengthScale(capacity),
        thickness=d.estimatedFlow?estimatedGatewayThicknessScale(d.estimatedAnnualGWh):importGatewayThicknessScale(capacity);
    ...
    importGateways.append('title').text(d=>d.estimatedFlow?`Conexión bidireccional ${d.estimatedFlow.from} ↔ ${d.estimatedFlow.to} · saldo probable ${d.estimatedFlow.from} → ${d.estimatedFlow.to}: ${formatNumber(d.estimatedAnnualGWh,0)} GWh en 2025 · ${formatNumber(d.flowRenewableShare,1)} % renovable / ${formatNumber(100-d.flowRenewableShare,1)} % no renovable`:`Conexión ${d.isFuture?'futura en construcción ':''}bidireccional · cruce fronterizo del corredor de ${d.name} · ${formatNumber(d.capacityTotalMVA,0)} MVA nominales · conexión con ${d.territory} · color agregado 2024: ${formatNumber(importRenewableShare,1)} % renovable y ${formatNumber(importNonRenewableShare,1)} % no renovable`);
    ...
    const zoom=d3.zoom().scaleExtent([1,9]).on('zoom',event=>{
      ...
      markers.attr('cx',d=>markerPosition(d,event.transform.k)[0]).attr('cy',d=>markerPosition(d,event.transform.k)[1]).attr('r',d=>(d.key==='wind'?windCapacityRadius(d.mw):d.mw<0.1?2.3:radius(d.mw))/Math.pow(event.transform.k,.62));
      ...
      ptsPotentialMarkers.attr('r',d=>(d.key==='wind'?windCapacityRadius(d.mw):ptsPotentialRadius(d.mw))/Math.pow(event.transform.k,.62));
    ...
    const defaultDetail=()=>{detail.innerHTML=`<h4>Infraestructura energética cartografiada</h4><p class="map-detail-lead">Selecciona una instalación, un proyecto, una zona PTS, un almacenamiento, una población, un nodo o un trazado para consultar sus datos.</p><dl><dt>Emplazamientos eléctricos en servicio</dt><dd>${formatNumber(totals.points,0)}</dd><dt>Potencia eléctrica en servicio</dt><dd>${formatNumber(totals.mw,1)} MW</dd><dt>Proyectos de generación</dt><dd>${formatNumber(generationProjectDefinitions.length,0)} autorizados o en obra</dd><dt>Zonas potenciales PTS</dt><dd>${formatNumber(ptsPotentialRecords.length,0)} oportunidades territoriales</dd><dt>Repotenciación eólica</dt><dd>${formatNumber(WIND_REPOWERING_POTENTIAL.length,0)} parques evaluados; 143,37 → 232,16 MW</dd><dt>Eólica marina · BiMEP</dt><dd>${formatNumber(OFFSHORE_WIND_POST_PTS.length,0)} ámbito de investigación; no suma potencia al modelo</dd><dt>Almacenamiento eléctrico</dt><dd>${formatNumber(storageRecords.length,0)} instalaciones y proyectos inventariados</dd><dt>Infraestructura de gas y petróleo</dt><dd>${formatNumber(fossilFacilityRecords.length,0)} instalaciones y ${formatNumber(nonElectricRouteRecords.length,0)} conducciones principales</dd><dt>Combustibles renovables</dt><dd>${formatNumber(renewableFuelFacilityRecords.length,0)} instalaciones</dd><dt>Biomasa no eléctrica</dt><dd>${formatNumber(biomassEnergy,1)} GWh en 2024; ${formatNumber(biomassFacilityRecords.length,0)} referencias cartografiadas</dd><dt>Población ${populationYear}</dt><dd>${formatNumber(populationTotal,0)} habitantes</dd><dt>Red eléctrica</dt><dd>132, 220 y 400 kV</dd><dt>Nodos eléctricos exteriores</dt><dd>${formatNumber(INTERCONNECTION_NODES.length,0)}</dd><dt>Inventario</dt><dd>${escapeHtml(electricStorage?.updated||nonElectricInfrastructure?.updated||ptsPotentialSites?.updated||generationProjects?.updated||electricProjects?.updated||'2026')}</dd></dl><p class="map-data-note">El almacenamiento se expresa por separado en MW (potencia) y MWh (energía almacenable); no se suma como generación ni modifica la barra energética. Gas y petróleo se muestran en gris. Los combustibles renovables se muestran en marrón y la biomasa térmica en marrón claro. La “P” azul identifica potencial de repotenciación evaluado por el PTS, no proyectos autorizados. El mini aerogenerador rojo de la capa eólica marina de BiMEP representa investigación que amplía el alcance territorial del PTS, no parques comerciales aprobados.</p><span class="map-source-badge">Red Eléctrica / ESIOS · GeoEuskadi · PTS · EVE · Gobierno Vasco · BiMEP · MITECO · Enagás · BBG · Petronor · Exolum</span>`;refreshDetailActions()};
    const showDetail=d=>{
      ...
      const technologyLabel=d.isEveSolar?'Solar a red · declarado por EVE':d.isDocumentedSelfConsumption?(d.properties.selfConsumptionLabel||'Autoconsumo documentado'):TECHNOLOGIES[d.key].label,sourceLabel=d.properties.detailSourceLabel||d.properties.fuente||(d.isEveSolar?'EVE':'Red Eléctrica / ESIOS');
    ...
    const importButton=document.createElement('button');importButton.type='button';importButton.className='map-import-toggle';importButton.setAttribute('aria-pressed','false');importButton.innerHTML='<i aria-hidden="true"></i>Capacidad de entrada de importación';importButton.title=useEstimatedGatewayFlows?'Mostrar u ocultar los flujos anuales estimados de 2025 con la misma escala que el mapa peninsular':'Mostrar u ocultar la capacidad de las conexiones exteriores por las que puede entrar electricidad a Euskadi';importButton.addEventListener('click',()=>{importsVisible=!importsVisible;updateNetworkVisibility()});
    ...
    const ptsPotentialButton=document.createElement('button');ptsPotentialButton.type='button';ptsPotentialButton.className='map-pts-potential-toggle';ptsPotentialButton.setAttribute('aria-pressed','false');ptsPotentialButton.innerHTML=`<i aria-hidden="true"></i>${focusedMarineMap?'Potencial PTS · energía oceánica':'Potencial PTS · nuevas instalaciones y repotenciación'}`;ptsPotentialButton.title=focusedMarineMap?'Mostrar u ocultar las doce zonas portuarias seleccionadas por el PTS para posible aprovechamiento de energía oceánica':'Mostrar u ocultar las Zonas de Localización Seleccionada y la repotenciación de parques eólicos existentes evaluada por el PTS';ptsPotentialButton.addEventListener('click',()=>{ptsPotentialVisible=!ptsPotentialVisible;updateNetworkVisibility();updateVisibility()});
    ...
    const standardFilterControls=[nonElectricButton,importButton,storageButton,filterButtons.get('cycle'),...COGEN_FUEL_CLASS_ORDER.map(key=>cogenFuelButtons.get(key)),renewableFuelButton,biomassButton,filterButtons.get('hydro'),filterButtons.get('wind'),filterButtons.get('solar'),isTendentialScenario?eveSolarButton:null,filterButtons.get('other'),generationProjectButton,ptsPotentialButton,offshoreWindPostPtsButton,populationButton,networkButton,substationButton];
    const focusedFilterControls=[...(focusedSolarMap&&showBothSolar?[eveSolarButton,filterButtons.get('solar')]:[onlyEveSolar?eveSolarButton:filterButtons.get(focusedTechnologyKey)]),(focusedWindMap||focusedSolarMap)?generationProjectButton:null,(focusedWindMap||focusedSolarMap||focusedMarineMap)?ptsPotentialButton:null,focusedWindMap?offshoreWindPostPtsButton:null,populationButton];
    ...
    const resetButton=document.createElement('button');resetButton.type='button';resetButton.className='map-reset-filters';resetButton.textContent='Restablecer filtros';resetButton.addEventListener('click',()=>{
      ...
      fossilGenerationActive=!infrastructureFocus&&(!focusedTechnologyKey||focusedTechnologyKey==='cycle');
      ...
      includeSmall=!infrastructureFocus;eveSolarVisible=!infrastructureFocus;eveSolarButton.setAttribute('aria-pressed',String(eveSolarVisible));smallButton.setAttribute('aria-pressed',String(includeSmall));smallButton.textContent=includeSmall?'Ocultar instalaciones pequeñas':'Mostrar instalaciones pequeñas';networkVisible=false;substationsVisible=false;substationExistingVisible=substationFocus;substationNewVisible=substationFocus;substationExpansionVisible=substationFocus;importsVisible=false;generationProjectsVisible=focusedWindMap||(!focusedTechnologyKey&&defaultGenerationProjectsVisible);ptsPotentialVisible=focusedWindMap||(!focusedTechnologyKey&&defaultPtsPotentialVisible);offshoreWindPostPtsVisible=offshoreWindFocus||focusedWindMap;storageVisible=storageFocus;populationsVisible=false;nonElectricVisible=false;renewableFuelVisible=false;biomassVisible=false;setElectricityExpanded(false);updateNetworkVisibility();updatePopulationVisibility();updateVisibility()
    ...
    function updateNetworkVisibility(){
      ...
      gridLayer.style('display',networkVisible?null:'none');nodeLayer.style('display',networkVisible?null:'none');local132Layer.style('display',networkVisible?null:'none');substationLayer.style('display',showExistingSubstations?null:'none');verifiedSubstationLayer.style('display',showExistingSubstations?null:'none');projectLayer.style('display',networkVisible||showAnySubstations?null:'none');projectPaths.style('display',networkVisible?null:'none');projectMarkers.style('display',record=>isSubstationProject(record.project)?(isNewSubstationProject(record.project)?(showNewSubstations?null:'none'):(showExpandedSubstations?null:'none')):(networkVisible?null:'none'));biscayLayer.style('display',networkVisible?null:'none');biscayNodeLayer.style('display',networkVisible?null:'none');importGatewayLayer.style('display',importsVisible?null:'none');generationProjectLayer.style('display',generationProjectsVisible?null:'none');ptsPotentialLayer.style('display',ptsPotentialVisible?null:'none');windRepoweringLayer.style('display',ptsPotentialVisible&&active.has('wind')?null:'none');offshoreWindPostPtsLayer.style('display',offshoreWindPostPtsVisible?null:'none');storageLayer.style('display',storageVisible?null:'none');nonElectricLayer.style('display',nonElectricVisible||renewableFuelVisible||biomassVisible?null:'none');nonElectricRoutes.style('display',nonElectricVisible?null:'none');nonElectricFacilityMarkers.style('display',facility=>facilityIsVisible(facility)?null:'none');nonElectricFacilityLeaders.style('display',facility=>facilityIsVisible(facility)?null:'none');importOriginVessel.style('display',nonElectricVisible?null:'none');renewableImportOriginVessel.style('display',renewableFuelVisible?null:'none');networkLegend.hidden=!(networkVisible||showAnySubstations||importsVisible||generationProjectsVisible||ptsPotentialVisible||offshoreWindPostPtsVisible||storageVisible||nonElectricVisible||renewableFuelVisible||biomassVisible);networkButton.setAttribute('aria-pressed',String(networkVisible));substationButton.setAttribute('aria-pressed',String(substationsVisible));substationExistingButton.setAttribute('aria-pressed',String(substationExistingVisible));substationNewButton.setAttribute('aria-pressed',String(substationNewVisible));substationExpansionButton.setAttribute('aria-pressed',String(substationExpansionVisible));importButton.setAttribute('aria-pressed',String(importsVisible));generationProjectButton.setAttribute('aria-pressed',String(generationProjectsVisible));ptsPotentialButton.setAttribute('aria-pressed',String(ptsPotentialVisible));offshoreWindPostPtsButton.setAttribute('aria-pressed',String(offshoreWindPostPtsVisible));storageButton.setAttribute('aria-pressed',String(storageVisible));nonElectricButton.setAttribute('aria-pressed',String(nonElectricVisible));renewableFuelButton.setAttribute('aria-pressed',String(renewableFuelVisible));biomassButton.setAttribute('aria-pressed',String(biomassVisible));energyShareBar.classList.toggle('is-non-electric-visible',nonElectricVisible);energyShareBar.classList.toggle('is-renewable-fuel-visible',renewableFuelVisible);energyShareBar.classList.toggle('is-biomass-visible',biomassVisible);
      networkLegend.querySelectorAll('[data-layer="grid"]').forEach(item=>item.hidden=!networkVisible);networkLegend.querySelectorAll('[data-layer="substations-existing"]').forEach(item=>item.hidden=!showExistingSubstations);networkLegend.querySelectorAll('[data-layer="substations-new"]').forEach(item=>item.hidden=!showNewSubstations);networkLegend.querySelectorAll('[data-layer="substations-expansion"]').forEach(item=>item.hidden=!showExpandedSubstations);networkLegend.querySelectorAll('[data-layer="imports"]').forEach(item=>item.hidden=!importsVisible);networkLegend.querySelectorAll('[data-layer="generation-projects"]').forEach(item=>item.hidden=!generationProjectsVisible);networkLegend.querySelectorAll('[data-layer="pts-potentials"]').forEach(item=>item.hidden=!ptsPotentialVisible);networkLegend.querySelectorAll('[data-layer="offshore-post-pts"]').forEach(item=>item.hidden=!offshoreWindPostPtsVisible);networkLegend.querySelectorAll('[data-layer="storage"]').forEach(item=>item.hidden=!storageVisible);networkLegend.querySelectorAll('[data-layer="non-electric"]').forEach(item=>item.hidden=!nonElectricVisible);networkLegend.querySelectorAll('[data-layer="renewable-fuels"]').forEach(item=>item.hidden=!renewableFuelVisible);networkLegend.querySelectorAll('[data-layer="thermal-biomass"]').forEach(item=>item.hidden=!biomassVisible);
      ...
      if(!offshoreWindPostPtsVisible&&offshoreWindPostPtsMarkers.filter('.is-selected').node()){clearOffshoreWindPostPtsSelection();defaultDetail()}
    ...
    function updateVisibility(){
      const visible=records.filter(d=>recordInScope(d)&&((d.isEveSolar&&usesEveSolarLayer)?eveSolarVisible:active.has(d.layerKey))&&(d.key!=='cogen'||activeCogenFuelClasses.has(d.cogenFuelClass))&&((d.isEveSolar&&usesEveSolarLayer)||(d.layerKey==='solar'&&!d.isEveSolar)||includeSmall||d.mw>=threshold)),visibleSet=new Set(visible);
      ...
      if(offshoreWindFocus)count.textContent=offshoreWindPostPtsVisible?'1 área experimental · 20 MW de conexión para ensayos · 0 MW comerciales adjudicados':'La capa de eólica marina está oculta';
    ...
    function updateEnergyShare(){
      ...
      ['wind','solar'].forEach(key=>{
        ...
        if(!segment)return;
        const isVisible=generationProjectsVisible&&isPotentialTechnologyActive(key);segment.hidden=!isVisible;
      ...
      ['wind','solar','other'].forEach(key=>{
        ...
        segment.title=`Potencial PTS de ${barTechnologyLabels[key].toLowerCase()}: ${formatNumber(ptsPotentialCapacityByTechnology[key],0)} MW agregados · base de la barra: ${basis}. El PTS no asigna potencia oficial a cada zona.${repoweringNote}${marineNote} El conjunto del potencial sustituye primero generación fósil y después electricidad importada.${deductedProjectEnergy?' Para evitar doble conteo se descuenta la energía de los proyectos autorizados o en obra.':''}${ptsScale<1?' La representación se limita a la generación fósil y la importación desplazables en la base 2024.':''}`;
        if(isVisible)selectedEnergy+=energy;
      });

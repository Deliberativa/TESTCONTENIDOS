(()=>{
  ...
  window.renderEnergyInstallationsMap=(row,cfg)=>{
    ...
    const energyShareTitle=energyShare.querySelector('.map-energy-share-title');energyShareTitle.innerHTML=`<strong>Total</strong><span data-energy-share-basis>${scenarioYear}</span>`;
  ...
  detail.prepend(button);
    ...
    const importLegendLabel=modelledGatewayFlows?'Importación modelizada 2050 · grosor proporcional a GWh':useEstimatedGatewayFlows?'Flujos estimados 2025 · grosor proporcional a GWh · escala visual ×2':`Capacidad de entrada de importación · ${formatNumber(importRenewableShare,0)} % renov. / ${formatNumber(importNonRenewableShare,0)} % no renov.`;
    ...
    const standardFilterControls=[nonElectricButton,importButton,storageButton,filterButtons.get('cycle'),...COGEN_FUEL_CLASS_ORDER.map(key=>cogenFuelButtons.get(key)),renewableFuelButton,biomassButton,filterButtons.get('hydro'),filterButtons.get('wind'),usesEveSolarLayer?eveSolarButton:null,filterButtons.get('solar'),filterButtons.get('other'),generationProjectButton,ptsPotentialButton,offshoreWindPostPtsButton,populationButton,networkButton,substationButton];
    ...
    const resetButton=document.createElement('button');resetButton.type='button';resetButton.className='map-reset-filters';resetButton.textContent='Restablecer filtros';resetButton.addEventListener('click',()=>{
      ...
      includeSmall=!infrastructureFocus;eveSolarVisible=!infrastructureFocus;eveSolarButton.setAttribute('aria-pressed',String(eveSolarVisible));smallButton.setAttribute('aria-pressed',String(includeSmall));smallButton.textContent=includeSmall?'Ocultar instalaciones pequeñas':'Mostrar instalaciones pequeñas';networkVisible=false;substationsVisible=false;substationExistingVisible=substationFocus;substationNewVisible=substationFocus;substationExpansionVisible=substationFocus;importsVisible=defaultImportsVisible;generationProjectsVisible=focusedWindMap||(!focusedTechnologyKey&&defaultGenerationProjectsVisible);ptsPotentialVisible=focusedWindMap||(!focusedTechnologyKey&&defaultPtsPotentialVisible);offshoreWindPostPtsVisible=offshoreWindFocus||focusedWindMap;storageVisible=storageFocus;populationsVisible=false;nonElectricVisible=false;renewableFuelVisible=false;biomassVisible=false;setElectricityExpanded(false);updateNetworkVisibility();updatePopulationVisibility();updateVisibility()
    ...
    function updateEnergyShare(){
      const potentialKeys=['wind','solar','other'],rawFutureEnergy=Object.values(futureBarTechnologyEnergy).reduce((sum,value)=>sum+value,0),futureScale=rawFutureEnergy?Math.min(1,importedElectricEnergyBase/rawFutureEnergy):0,projectEnergyByTechnology=Object.fromEntries(potentialKeys.map(key=>[key,!generationExpansionIncludedInBase&&generationProjectsVisible&&isPotentialTechnologyActive(key)?(futureBarTechnologyEnergy[key]||0)*futureScale:0])),futureEnergy=Object.values(projectEnergyByTechnology).reduce((sum,value)=>sum+value,0),importedAfterProjects=Math.max(0,importedElectricEnergyBase-futureEnergy),cycleEnergyBase=barTechnologyEnergy.cycle||0;
      ...
      if(importedSegment){importedSegment.style.flex=String(importedElectricEnergy);importedSegment.classList.toggle('is-inactive',!importsVisible);importedSegment.title=`Electricidad importada${importsVisible?'':' (capa de entradas oculta)'}: ${formatNumber(importedElectricEnergy,0)} GWh · ${formatNumber(displayedElectricEnergy?100*importedElectricEnergy/displayedElectricEnergy:0,1)} % de la electricidad · atribución ${scenarioYear}: ${formatNumber(importRenewableShare,1)} % renovable y ${formatNumber(importNonRenewableShare,1)} % no renovable${generationProjectsVisible&&!generationExpansionIncludedInBase?' · reducida por el potencial anual estimado de los proyectos solares y eólicos autorizados o en construcción':''}${ptsPotentialVisible?' · reducida además por el potencial territorial PTS visible, sin doble contabilizar los proyectos':''}`}
      if(basisLabel){
        const exploratoryLayers=[generationProjectsVisible?'construcción':null,ptsPotentialVisible?'zonas potenciales':null].filter(Boolean);
        basisLabel.textContent=isNormativeScenario?`Escenario normativo · 2050${allowExploratoryGenerationLayers&&exploratoryLayers.length?` + ${exploratoryLayers.join(' + ')} · exploración`:''}`:generationProjectsVisible&&ptsPotentialVisible?`Base ${scenarioYear} + proyectos + potencial PTS`:generationProjectsVisible?`Base ${scenarioYear} + proyectos`:ptsPotentialVisible?`Base ${scenarioYear} + potencial PTS`:String(scenarioYear);
      }

(()=>{
  ...
  window.renderEnergyInstallationsMap=(row,cfg)=>{
    ...
    const biscayFeatures=biscayInterconnector?.features||[],biscayMetadata=biscayInterconnector?.metadata||{},projectDefinitions=electricProjects?.projects||[],generationProjectDefinitions=generationProjects?.projects||[],ptsPotentialDefinitions=ptsPotentialSites?.sites||[];
    ...
    const barTechnologyLabels={...Object.fromEntries(TECHNOLOGY_ORDER.map(key=>[key,TECHNOLOGIES[key].label])),cogen:'Cogeneración, residuos y biomasa'},barSegmentDisplayLabels={cycle:'Ciclo<br>combinado',cogen:'Cogeneración',hydro:'Hidráulica',wind:'Eólica',solar:'Solar',other:'Marina'};
    const technologySegments=barTechnologyKeys.map(key=>{
      ...
      const currentSegment=`<${tag}${buttonAttributes} class="map-energy-share-segment${isDrillable?' map-energy-share-drillable':''}" data-technology="${key}" style="--segment-color:${TECHNOLOGIES[key].color};background:${TECHNOLOGIES[key].color};flex:${energy}" title="${barTechnologyLabels[key]}: ${formatNumber(energy,energyDigits)} GWh · ${formatNumber(electricPercent,key==='other'?3:1)} % de la electricidad · ${formatNumber(totalPercent,key==='other'?4:2)} % del consumo final"><span class="map-energy-share-segment-label">${barSegmentDisplayLabels[key]}</span>${expandedLabel}${solarBreakdown}</${tag}>`;
    ...
    const energyShareBar=energyShare.querySelector('.map-energy-share-bar'),electricFocusButton=energyShare.querySelector('.map-energy-share-electric-toggle'),restShareBlock=energyShare.querySelector('.map-energy-share-rest'),electricShareBlock=energyShare.querySelector('.map-energy-share-electric'),restShareStrong=energyShare.querySelector('.map-energy-share-rest strong'),technologyDrillButtons=[...energyShare.querySelectorAll('[data-drilldown-technology]')];let expandedTechnology=null;
    ...
    const detailModalBody=detailModal.querySelector('.map-detail-modal-body'),detailModalClose=detailModal.querySelector('.map-detail-modal-close');
    const refreshDetailActions=()=>{
      ...
      button.addEventListener('click',()=>{
        const copy=detail.cloneNode(true);copy.querySelector('.map-detail-more')?.remove();detailModalBody.replaceChildren(...copy.childNodes);detailModal.hidden=false;detailModalClose.focus();
      });
  detail.prepend(button);
    ...
    const scope=document.createElement('p');scope.className='map-scope';scope.innerHTML='<strong>Cómo leer las conexiones.</strong> Las líneas terminan en subestaciones; los círculos lisos son instalaciones en servicio y los círculos con rayas negras pertenecen a la capa independiente de generación autorizada o en construcción. Los círculos cuadriculados muestran Zonas de Localización Seleccionada del PTS: son oportunidades territoriales, no proyectos. Su color indica la tecnología. La red de transporte se representa a 220 y 400 kV y la capa secundaria añade las conexiones locales de 132 kV. El color sigue indicando la tensión y el trazo discontinuo identifica obras de red en construcción o autorizadas. La nueva interconexión con Francia llega soterrada desde Gatika y continúa por el Golfo de Bizkaia. <strong>Poblaciones.</strong> Los cuadrados negros sitúan los municipios según su población a 1 de enero de 2025.';shell.append(scope);
    ...
    const defaultDetail=()=>{detail.innerHTML=`<h4>Infraestructura eléctrica cartografiada</h4><p class="map-detail-lead">Selecciona una instalación, un proyecto, una zona PTS, una población, un nodo o un trazado discontinuo para consultar sus datos.</p><dl><dt>Emplazamientos en servicio</dt><dd>${formatNumber(totals.points,0)}</dd><dt>Instalaciones en servicio</dt><dd>${formatNumber(totals.units,0)}</dd><dt>Potencia en servicio</dt><dd>${formatNumber(totals.mw,1)} MW</dd><dt>Proyectos de generación</dt><dd>${formatNumber(generationProjectDefinitions.length,0)} autorizados o en obra</dd><dt>Zonas potenciales PTS</dt><dd>${formatNumber(ptsPotentialRecords.length,0)} oportunidades territoriales</dd><dt>Población ${populationYear}</dt><dd>${formatNumber(populationTotal,0)} habitantes</dd><dt>Red de transporte</dt><dd>220 y 400 kV</dd><dt>Conexiones locales</dt><dd>132 kV</dd><dt>Subestaciones</dt><dd>${formatNumber(mapSubstations?.features?.length||0,0)}</dd><dt>Nodos exteriores</dt><dd>${formatNumber(INTERCONNECTION_NODES.length,0)}</dd><dt>Obras de red</dt><dd>${formatNumber(activeProjectCount,0)} actuaciones</dd><dt>Inventario</dt><dd>${escapeHtml(ptsPotentialSites?.updated||generationProjects?.updated||electricProjects?.updated||'2026')}</dd></dl><p class="map-data-note">Los círculos rayados son proyectos autorizados o en obra. Los círculos cuadriculados son Zonas de Localización Seleccionada del PTS: indican aptitud territorial, no proyectos ni potencias oficiales asignadas a cada zona.</p><span class="map-source-badge">Red Eléctrica / ESIOS · GeoEuskadi · PTS de Energías Renovables · INELFE · Gobierno Vasco · BOE · Eustat</span>`;refreshDetailActions()};
    const showDetail=d=>{
      markers.classed('is-selected',candidate=>candidate===d);
      populationGroups.classed('is-selected',false);networkNodes.classed('is-selected',false);biscayPaths.classed('is-selected',false);clearProjectSelection();clearGenerationProjectSelection();clearPtsPotentialSelection();
    ...
    const showPopulationDetail=record=>{
      markers.classed('is-selected',false);populationGroups.classed('is-selected',candidate=>candidate===record);networkNodes.classed('is-selected',false);biscayPaths.classed('is-selected',false);clearProjectSelection();clearGenerationProjectSelection();clearPtsPotentialSelection();
    ...
    const showNetworkDetail=d=>{
      markers.classed('is-selected',false);populationGroups.classed('is-selected',false);networkNodes.classed('is-selected',candidate=>candidate===d);biscayPaths.classed('is-selected',false);clearProjectSelection();clearGenerationProjectSelection();clearPtsPotentialSelection();
    ...
    const showGenerationProjectDetail=project=>{
      markers.classed('is-selected',false);populationGroups.classed('is-selected',false);networkNodes.classed('is-selected',false);biscayPaths.classed('is-selected',false);clearProjectSelection();clearPtsPotentialSelection();
    ...
    const showPtsPotentialDetail=site=>{
      markers.classed('is-selected',false);populationGroups.classed('is-selected',false);networkNodes.classed('is-selected',false);biscayPaths.classed('is-selected',false);clearProjectSelection();clearGenerationProjectSelection();
    ...
    const showBiscayInterconnectorDetail=()=>{
      markers.classed('is-selected',false);populationGroups.classed('is-selected',false);networkNodes.classed('is-selected',false);clearProjectSelection();clearGenerationProjectSelection();clearPtsPotentialSelection();biscayPaths.classed('is-selected',true);
    ...
    ptsPotentialMarkers.on('mouseenter focus click',(event,site)=>{if(event.type==='click')event.stopPropagation();showPtsPotentialDetail(site)}).on('keydown',(event,site)=>{if(event.key==='Enter'||event.key===' '){event.preventDefault();showPtsPotentialDetail(site)}});
    ...
    svg.on('click',event=>{if(event.target===svg.node()){markers.classed('is-selected',false);populationGroups.classed('is-selected',false);networkNodes.classed('is-selected',false);biscayPaths.classed('is-selected',false);clearProjectSelection();clearGenerationProjectSelection();clearPtsPotentialSelection();defaultDetail()}});
    defaultDetail();

    const active=new Set(TECHNOLOGY_ORDER),threshold=.1;let includeSmall=true,networkVisible=false,local132Visible=false,projectsVisible=false,generationProjectsVisible=false,ptsPotentialVisible=false,populationsVisible=false;
    ...
    function updateEnergyShare(){
      ...
      barTechnologyKeys.forEach(key=>{
        const segment=energyShare.querySelector(`[data-technology="${key}"]`),isActive=active.has(key),energy=barTechnologyEnergy[key],displayEnergy=key==='cycle'?Math.max(0,energy-cycleDisplacement):energy;
        if(!segment)return;
        ...
        segment.title=`${barTechnologyLabels[key]}${isActive?'':' (capa oculta)'}: ${formatNumber(displayEnergy,key==='other'?3:0)} GWh · ${formatNumber(electricEnergy?100*displayEnergy/electricEnergy:0,key==='other'?3:1)} % de la electricidad${key==='cycle'&&cycleDisplacement?` · ${formatNumber(cycleDisplacement,0)} GWh sustituidos por el potencial PTS visible`:''}${smallNote}`;
      ...
      ['wind','solar','other'].forEach(key=>{
        ...
        segment.title=`Potencial PTS de ${barTechnologyLabels[key].toLowerCase()}: ${formatNumber(ptsPotentialCapacityByTechnology[key],0)} MW agregados · base de la barra: ${basis}. El PTS no asigna potencia oficial a cada zona.${marineNote} El conjunto del potencial sustituye primero ciclo combinado y después electricidad importada.${deductedProjectEnergy?' Para evitar doble conteo se descuenta la energía de los proyectos autorizados o en obra.':''}${ptsScale<1?' La representación se limita a la generación de ciclo combinado y la importación desplazables en la base 2024.':''}`;
        if(isVisible)selectedEnergy+=energy;
      });
      ...
      if(bar){bar.classList.toggle('is-small-hidden',!includeSmall);bar.setAttribute('aria-label',`Electricidad: ${formatNumber(displayedElectricShare,1)} % del consumo final. Energía no eléctrica, principalmente petróleo y gas natural: ${formatNumber(displayedNonElectricShare,1)} %. Las capas de generación visibles representan ${formatNumber(selectedEnergy,0)} GWh; la capa de red de transporte ${networkVisible?'muestra':'oculta'} ${formatNumber(importedElectricEnergy,0)} GWh importados.${generationProjectsVisible?` Los proyectos solares y eólicos visibles añaden ${formatNumber(futureEnergy,0)} GWh potenciales y reducen importación.`:''}${ptsPotentialVisible?` Las ZLS del PTS añaden visualmente ${formatNumber(ptsEnergy,0)} GWh de potencial territorial; sustituyen primero ${formatNumber(cycleDisplacement,0)} GWh de ciclo combinado y después ${formatNumber(ptsImportDisplacement,0)} GWh de importación. Los proyectos ya representados se descuentan para evitar doble conteo. Esta exploración no modifica las proyecciones del modelo.`:''}`)}
    }
